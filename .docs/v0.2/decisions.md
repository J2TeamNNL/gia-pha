...
| ADR-005 | Superseded by ADR-018 | Use React Flow plus ELK worker with focused rendering. | Genealogy is not a single-root tree; the combination supports graph viewport interaction, ports, and complex layered layout. |
...
| ADR-018 | Accepted | The graph is a hand-written SVG/DOM canvas over a plain layout Worker; neither React Flow nor ELK is a dependency. | Recorded 2026-08-16 to correct ADR-005, which described an intent the shipped `UI-002` never implemented. `src/graph/layout.ts` and `src/components/GraphView.tsx` carry the layout and viewport themselves, and `package.json` has no `reactflow` or `elkjs` entry. Keeping the two libraries in the documentation misdirected every agent that read it. |
| ADR-019 | Accepted | Bulk writes go through one `batch` worker command wrapped in `BEGIN IMMEDIATE`/`COMMIT`, with per-row validation performed on the main thread before the batch is sent. | A pasted list or an imported file is one user intent and must not half-apply. Validation stays where `validateRelationship` already lives, and the worker stays a dumb executor rather than growing genealogy rules. |
